...
describe('adminQuizDescriptionUpdate Successful Tests', () => {
  ...
  test("Editing quiz2's description, twice", () => {
    ...
    const quizInfo = requestQuizInfo(user1.token, quiz2.quizId).body;

    expect(quizInfo.timeCreated).toBeGreaterThanOrEqual(timeCreated2 - 1);
    expect(quizInfo.timeCreated).toBeLessThanOrEqual(timeCreated2 + 2);
    expect(quizInfo.timeLastEdited).toBeGreaterThanOrEqual(timeLastEdited2 - 1);
    expect(quizInfo.timeLastEdited).toBeLessThanOrEqual(timeLastEdited2 + 2);
  });
});
