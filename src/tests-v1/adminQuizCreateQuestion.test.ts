...
describe('Successful tests (POST /v1/admin/quiz/{quizid}/question)', () => {
  test('Successful question creation', () => {
    ...
    const response = requestQuizInfo(userId.token, quizId.quizId);
    const { body } = response;
...
describe('Unsuccessful tests (POST /v1/admin/quiz/{quizid}/question)', () => {
  test('Invalid token', () => {
    expect(
      reqQuizCreateQuestion(
        userId.token + 1,
